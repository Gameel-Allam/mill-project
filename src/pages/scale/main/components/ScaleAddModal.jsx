...
const ScaleAddModal = () => {
  ...
  const [premissible, setpremissibleLoss] = useState('')
  const [LossWarning, setLossWarning] = useState('')
  const calculateLoss = () => {
    const premissibleLoss = formik.values.portRealWeight * .0003;
    const actualWeight = formik.values.carWeightWithLoad - formik.values.carWeightEmpty;
    const importedWeight = formik.values.portRealWeight;
    const lossInWeight = importedWeight - actualWeight;
    setLoss(lossInWeight.toFixed(2) + " كيلو")
    setpremissibleLoss(premissibleLoss.toFixed(2) + " كيلو")
    if (lossInWeight > premissibleLoss) {
                  ...
                  <>
                    ...
                    <span className="d-flex flex-row justify-content-start align-items-center my-3">
                      ...
                      <input
                        ...
                        dir="rtl"
                        value={premissible}
                        name="premissibleLoss"
                        className={`col-3 ${styles.disabled__field}`}
                        disabled
                      />
                      {/* className={styles.loss__error} */}
                    </span>
